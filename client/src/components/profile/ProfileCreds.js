    ...
    render() {
        ...
        const expItems = experience.map(exp => (
            <li key={exp._id} className="list-group-item">
                <h4>{exp.company}</h4>
                <p>
                    <Moment format="MMMM DD, YYYY">{exp.from}</Moment> - {exp.to === null ? "Present" : <Moment format="MMMM DD, YYYY">{exp.to}</Moment>}
                </p>
        ...
        const eduItems = education.map(edu => (
            <li key={edu._id} className="list-group-item">
                <h4>{edu.school}</h4>
                <p>
                    <Moment format="MMMM DD, YYYY">{edu.from}</Moment> - {edu.to === null ? "Present" : <Moment format="MMMM DD, YYYY">{edu.to}</Moment>}
                </p>
